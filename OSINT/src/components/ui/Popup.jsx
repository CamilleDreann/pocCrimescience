...
const TYPE_COLORS = {
  ...
  info: 'var(--color-accent, #e95420)',
}

export default function Popup({ title, message, type = 'info', onClose, actions }) {
  const accentAttr = { style: { '--popup-accent': TYPE_COLORS[type] || TYPE_COLORS.info } }
      ...
      <div className={styles.popup} onClick={e => e.stopPropagation()}>
        ...
        <div className={styles.header}>
          <div className={styles.headerTitle}>
            <Icon name={TYPE_ICONS[type] || 'info'} size={20} color="currentColor" />
            <span>{title}</span>
          </div>
