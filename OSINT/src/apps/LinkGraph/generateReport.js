...
const NODE_W = 160, NODE_H = 40, PERSON_W = 180, PERSON_H = 48, CUSTOM_W = 160, CUSTOM_H = 40
...
function buildSVG(nodes, edges) {
  ...
  const edgeSVG = edges.map(edge => {
    ...
    const edgeColor = toNode.type === 'custom'
      ? toNode.data.color
  ...
  const nodeSVG = nodes.map(node => {
    if (node.type === 'person') {
      const initials = node.data.name.split(' ').map(w => w[0]).join('')
      const emailShort = node.data.email.length > 22 ? node.data.email.slice(0, 22) + '...' : node.data.email
      return `
    ...
    if (node.type === 'custom') {
      ...
      const titleShort = node.data.title.length > 14 ? node.data.title.slice(0, 14) + '…' : node.data.title
      const contentLabel = node.data.contents.length > 0
    ...
    const usernameShort = node.data.username.length > 16 ? node.data.username.slice(0, 16) + '...' : node.data.username
    return `
      <g transform="translate(${node.x}, ${node.y})">
...
function buildNodeCards(nodes) {
  return nodes.map(node => {
    ...
    // custom node
    const col = node.data.color || '#3fb950'
    const contentsHTML = node.data.contents.map(item => {
      if (item.type === 'text') {
